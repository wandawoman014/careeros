...
function cleanAnswerText(answer: string) {
  return answer
    ...
    .filter((line) => {
      ...
      return !/^(ROUTE|run_id|company_id|sheet name|sheet|source table|generated by|visual_spec|raw source|careeros query|report routing)\s*:/i.test(trimmed);
    })
    .join("\n")
    .trim();
}
...
function summarizeCareerBrief(answer: string) {
  ...
  return {
    ...
    nextMove: takeFirstSentence(paragraphs[2]?.text || listItems[3] || listItems[0] || ""),
  };
}
